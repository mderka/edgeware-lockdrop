...
contract('Lockdrop', (accounts) => {
  ...
  it('should ensure base58 encodings are valid to submit', async function () {
    await Promise.all(accounts.map(async (a, inx) => {
      return await lockdrop.lock(TWELVE_MONTHS, fixtures[inx].pubKey, (Math.random() > 0.5) ? true : false, {
        from: a,
    ...
    console.log(totalETHLockedInETH, totalETHSignaledInETH)
